...
const CHANGELOG=[
  {ver:'v5.67',date:'2026-07-19',items:['⚡ <b>Lightning API key (WarPulse)</b><br>Settings → ⚡ Lightning now has a field to paste your own <a href="https://lightning.warpulse.com" target="_blank" rel="noopener">lightning.warpulse.com</a> API key, exactly like the AI Assistant and Skylink keys. It\'s stored only on your device (localStorage) and never sent anywhere except WarPulse. (Live-strike data wiring comes next; today it just securely holds the key.)'],},
  {ver:'v5.66',date:'2026-07-19',items:['🎯 <b>Storm-track cone count now breaks down by tier</b><br>The header used to say only "You are in 2 storm track cones," which didn\'t explain why just one cell showed in the list. It now splits the cones you\'re inside by the same cross-track tiers as the badges, e.g. <b>"🎯 In 2 storm tracks · Direct: 1, Nearby: 1."</b> A cone counts as Direct only when its center threads within 1.5&nbsp;mi of you; wider ones are Nearby (1.5–6&nbsp;mi) or Passing (&gt;6&nbsp;mi), which is why "Approaching only" hides them. The "hidden by filters" note is also fixed — instead of the contradictory "none head directly at you," it now reads e.g. "1 heading directly, 1 passing wider; uncheck Approaching only to see the rest."'],},
  {ver:'v5.65',date:'2026-07-19',items:['⏱️ <b>Live distance & countdown in the storm sentence</b><br>The plain-English readout\'s distance and arrival now tick every second like the boxes do — the distance counts down as the cell closes in and the arrival shows a live countdown next to the clock time (e.g. "arriving ~2:14 PM (in 0m:31s)"). It also fixes a mismatch where the sentence showed the older scan distance while the Distance box showed the live one — both now agree.'],},
  {ver:'v5.64',date:'2026-07-19',items:['📝 <b>Plain-English storm readout + simpler threat</b><br>Each storm card now opens with one sentence that ties the whole picture together, e.g. <i>"⛈️ Strong storm — 26.7 mi to your N (12°), moving S (188°) at 12 mph. 🎯 Direct impact, XTK 0.2 mi, arriving ~3:23 PM · Heavy rain (~48 dBZ) expected."</i> (arrival time follows your 12/24-hour setting). A view button in the Sort/Filter bar flips between <b>Hybrid</b> (text + boxes), <b>Text</b> only, and <b>Cards</b> (boxes only).<br>• <b>Threat score</b> is now purely how directly the track crosses you: 10 at a bullseye, −4 per mile of cross-track (9.2 at 0.2&nbsp;mi, 4 at 1.5&nbsp;mi, 0 by 2.5&nbsp;mi). Storm strength is shown separately by the rain words, Peak dBZ, and the cell name.<br>• <b>"Min dBZ" filter and "Strongest" sort</b> now use the estimated dBZ at you (track-aware), so a strong cell passing wide no longer tops the list.<br>• Estimated rain below 15&nbsp;dBZ reads "little/no rain at you" instead of a tiny, drift-sensitive number.'],},
  {ver:'v5.63',date:'2026-07-19',items:['🎯 <b>Threat, estimated dBZ & "Approaching" now all use the 1.5-mi cross-track scale</b><br>Three things now key off the same cross-track (X-TRK) distance — how far a cell\'s projected path passes from you:<br>• <b>Estimated dBZ at you</b> follows a 1.5-mi falloff: 0.0&nbsp;mi=100% · 0.5&nbsp;mi=80% · 1.0&nbsp;mi=60% · 1.5&nbsp;mi=40% (est&nbsp;=&nbsp;peak&nbsp;dBZ&nbsp;×&nbsp;%). A 55&nbsp;dBZ cell passing 1.8&nbsp;mi to your side now reads ~15&nbsp;dBZ at you instead of a near-full 52.<br>• <b>Threat score</b> scales with that same closeness, so a cell heading over you outranks an equally strong one passing wide.<br>• <b>"Approaching only" filter</b> now means heading directly at you (X-TRK ≤ 1.5&nbsp;mi), matching the 🎯 Direct badge — it was using the old ≤12-mi rule.<br>Nearby cells still appear (with honest lower dBZ); only their strength-at-you is dialed down.'],},
  {ver:'v5.62',date:'2026-07-19',items:['🎯 <b>Simpler, honest storm-cell proximity</b><br>Storm cards used to show a "DIRECT 88%" badge from an old 6-tier scale — so a cell whose track passes 2.6&nbsp;mi to your side still read "DIRECT". The badge is now driven purely by the cross-track (X-TRK) distance on one clean scale: <b>🎯 Direct ≤1.5&nbsp;mi · 🟠 Nearby 1.5–6&nbsp;mi · ⚪ Passing >6&nbsp;mi</b>. The confusing closeness "%" is gone (the X-TRK tile already gives the exact miles), and the duplicate "Strength at you" tile was removed — the strength estimate now lives in one place: the "Est. at you: NN dBZ (rain)" line.'],},
  {ver:'v5.61',date:'2026-07-19',items:['🌧️ <b>Clearer rain timing in push notifications</b><br>The rain line in an alert used to read "rain now → soon", which looked like rain now <i>and then</i> a second, future round — and "soon" was vague. It now shows the real end time: "rain now, ends 1:32 PM". The narrated version also gives the exact clock ("ending around 1:32 PM") instead of "in a few minutes" whenever your time zone is known. Inbound rounds already showed exact start–end times and are unchanged.'],},
  {ver:'v5.60',date:'2026-07-19',items:['📍 <b>GPS locate now retries instead of just giving up</b><br>On a regular desktop browser (and sometimes on phones), the very first high-accuracy GPS request can fail transiently — the OS location service hasn\'t warmed up yet — returning "position unavailable" or timing out even though you already granted permission and no allow-location prompt ever appeared. Before, that first failure ended the attempt with an error toast, so "Find my location" looked broken until you tapped it a second time. It now automatically retries once with a lower-accuracy, longer-timeout fix (network/IP based — faster and more reliable), so a single tap succeeds. A genuine permission denial still tells you to enable location in your browser/phone settings.'],},
  {ver:'v5.59',date:'2026-07-19',items:['🎯 <b>Fixed "raining now" when the rain was miles away</b><br>The conditions hero could show "Light–moderate rain · LIVE RADAR" while the radar and Rain Clock agreed the nearest precipitation was several miles off. The overhead check was trusting each radar echo\'s cached distance-from-scan-center, but those points can come from different scan centers (the quick overhead refresh merges into the last full scan) — so a genuinely-distant cell could carry a stale "close" distance and read as overhead. It now measures the true distance from your exact spot to every echo, so only rain within ~1.5 mi counts as on you; anything farther shows as "Nearest Precipitation" instead. The hero, Rain Clock, and radar now agree.'],},
  {ver:'v5.58',date:'2026-07-19',items:['🔮 <b>Future storm names — full name lists</b><br>The projected "Future:" name for an unnamed depression now works for every season through the whole 6-year rotation (Atlantic + East Pacific), using the official NHC lists with the latest retired-name replacements (Melissa→Molly, Beryl→Brianna, Helene→Holly, Milton→Miguel; Pacific Otis→Otilio, Dora→Debora, John→Jake).'],},
  {ver:'v5.57',date:'2026-07-19',items:['🔮 <b>"Future" name for unnamed depressions</b><br>A tropical depression doesn\'t get a real name until it strengthens to a tropical storm — until then it\'s just "Tropical Depression Two." StormTracker now projects the name it would take next, pulled from the official rotating list for its basin (Atlantic or East Pacific), and shows it as <b>Future: …</b> on the storm card, the map pop-up, and the tropical banner. For example, this season\'s TD Two shows <b>Future: Bertha</b>. It\'s an estimate — the actual name can shift if an earlier depression never gets named — and it disappears the moment the storm is officially named.'],},
  {ver:'v5.56',date:'2026-07-19',items:['🌀 <b>Fixed the wrong storm showing "IN CONE"</b><br>The tropical banner and each storm\'s "IN CONE" tag could latch onto the wrong system — for example a distant Eastern-Pacific storm off Baja reading as if you were inside its forecast cone from Florida, even though the background push notification correctly named the nearby Atlantic system. The in-app cone match now uses the same strict rule as the push scanner (it will never pair a storm with a blank/mismatched id to the first cone it finds), plus a sanity check that a storm\'s own position actually sits within its cone. The tropical banner now always features the CLOSEST system you\'re truly in the cone of, instead of whichever one happened to be last in the list.'],},
  {ver:'v5.55',date:'2026-07-14',items:['📍 <b>Smarter address search</b><br>Messy or redundant addresses now work — duplicate city/state parts, stray fragments (like "TN 3" left over from a bad autofill), and mixed-up ordering are cleaned up automatically. If an exact match can\'t be found, StormTracker retries with simpler versions of your address and, for US street addresses, checks the US Census geocoder as a backup. Results that land in the wrong city or state are rejected instead of silently accepted, and when only the road can be pinpointed, your location label keeps the city and zip you typed instead of showing just the county. Worldwide addresses (UK, Canadian, European formats) get the same cleanup.<br><br>🔎 <b>Search troubleshooter</b><br>If an address still can\'t be found, the location panel now shows exactly what was tried — each retry, what the map services answered, and why results were rejected — so you can see what to fix. The welcome screen\'s search box uses the same smarter search too.<br><br>🔧 <b>Bug fix</b><br>Fixed a crash that could interrupt a search when the app switched between °F/mph and °C/km/h for a new region (for example, picking a Canadian address while using US units).'],},
  {ver:'v5.54',date:'2026-07-07',items:['📉 <b>One 25 dBZ rain floor for the Rain Clock + graph</b><br>The Rain Clock (both the radar and forecast rings) and the "Total Precipitation Next 36 hrs" graph now share a single rain floor of 25 dBZ (about 0.05 in/hr) — anything lighter is filtered out, so drizzle and trace no longer clutter them. (Raised from the old 15 dBZ radar floor.) Your storm-alert "Min strength" setting (40 dBZ) is separate and unchanged.'],},
  {ver:'v5.53',date:'2026-07-07',items:['📶 <b>Adapts to your connection speed</b><br>StormTracker now measures your connection at startup and re-checks every 60 seconds in the background, then adjusts automatically: on a slow link it eases off background radar updates and shows a small 🐢 Slow / 🐌 Very slow / 📡 Offline chip in the header so you know why. When your connection recovers, it returns to full speed on its own. The speed check also now reports a slow-but-working link as "slow" instead of "connection failed".'],},
  {ver:'v5.52',date:'2026-07-07',items:['⚡ <b>Faster first load</b><br>The map library (Leaflet) was loading as a blocking script in the page head even though the map is only used on the Radar tab. It now loads deferred, so the Weather tab paints sooner — most noticeable on a cold start over cellular. Also added connection warm-up hints for the external font, map, and tile services.'],},
  {ver:'v5.51',date:'2026-07-07',items:['🌧️ <b>Rain Clock: forecast stays on the inner ring</b><br>The dashed forecast ring and the solid radar ring are always on separate rings now (forecast inside, radar outside), so they never overlap or conflict. Before, when radar showed nothing, the forecast would jump out to the outer ring as a solid band; now it stays on the inner ring in every case. Also tidied the card header — removed the RADAR/TEXT labels (tap the dial centre to switch to the text summary).'],},
  {ver:'v5.50',date:'2026-07-07',items:['🌧️ <b>Rain Clock forecast ring now shows light rain</b><br>The dashed inner forecast ring only lit up for moderate-or-heavier forecast rain (a 28 dBZ floor), so with light rain in the forecast the dual ring looked like it was not working even though the 36-hour bar chart showed the rain. The forecast ring now uses a light-rain floor (~0.008 in/hr), so it appears whenever measurable rain is in the forecast — matching the bar chart.'],},
  {ver:'v5.49',date:'2026-07-07',items:['🎯 <b>"Raining now" now reads your exact spot</b><br>The overhead reading (the Rain Clock now-band, the conditions card, and rain-overhead alerts) was counting rain in the neighbouring map hex — up to a few miles away — as raining on you, so the clock could say "Rain until …" while nothing was actually over your location. It now only counts a radar echo within about 1.5 miles as overhead; rain in the 1.5–6 mile band shows up as "Nearest Precipitation" instead of a false raining-now.'],},
  {ver:'v5.48',date:'2026-07-07',items:['🌧️ <b>Rain Clock — forecast + radar together</b><br>The Rain Clock now shows two rings at once: the solid OUTER ring is live radar (observed storms and when they will actually reach you), and a new dashed INNER ring is the hourly forecast (predicted rain). Now you can compare when the forecast expects rain against when radar has it arriving. A "Span: fits forecast ⇄" chip in the dial legend switches between growing the dial so forecast rain stays in view, or keeping it sized to the nearest radar storm.'],},
  {ver:'v5.47',date:'2026-07-07',items:['🎯 <b>One "raining here now" answer</b> (Phase 4, final phase of the scan-pipeline consolidation) — the weather card, the Rain Clock\'s "raining now" reading, and the rain-overhead alerts now all read the exact same radar measurement at your exact spot. Before, the Rain Clock could show a heavier intensity than the weather card (it also consulted the nearest storm cell\'s overall strength). Now all three surfaces always show the same number. Side effect: the clock\'s "rain until…" estimate can end a little earlier, since it\'s now sized from the rain directly over you rather than the whole storm cell.'],},
  {ver:'v5.46',date:'2026-07-07',items:['🧩 <b>One post-scan pipeline</b> (Phase 3 of the scan-pipeline consolidation) — after any radar scan finishes, the app updates a dozen things at once: storm cards, badges, the mini-sonar, the hero card, the rain clock, map markers and zones, the threat ticker. Each of the four scan paths had its own copy of that update list, which is how they drift out of sync over time. Now there is exactly one, shared by all four. No visible changes — every scan now updates every display the exact same way.'],},
  {ver:'v5.45',date:'2026-07-07',items:['🧩 <b>One radar scan engine</b> (Phase 2 of the scan-pipeline consolidation) — the four ways the app scans radar (your location scan, the map "Scan View" button, the Hi-Res scan, and the quick every-few-minutes overhead rain check) each had their own copy of the tile-scanning code. They now all share one engine. Two real fixes come along for free: a long-standing race where scanning the map view while a location scan was running could briefly corrupt storm distances is gone (the scan center is now passed explicitly instead of temporarily overwriting your location), and the radar-frames index is fetched once per minute instead of separately by every scan path. No visible changes — groundwork for smarter storm tracking.'],},
  {ver:'v5.44',date:'2026-07-07',items:['🎯 <b>"In 30 storm cones but nothing approaching" explained</b> — the Storms tab could say "You are currently in 30 storm track cones" while a green "No storms currently approaching" sat right below it, the list showed 0 storms, and the badge read 0 — which looked broken. It wasn\'t: being "in a cone" means you\'re inside a storm\'s broad projected track fan (which gets very wide far away — about 13 miles half-width at 50 miles out), while "approaching" is a much stricter bar (projected to pass within ~6 miles of you). Now, when you\'re in cones but nothing meets that strict bar: the banner turns amber and explains it, including the closest projected pass distance; the "cells hidden by filters" note tells you it\'s the "Approaching only" filter hiding them; and the Rain Clock\'s "Nearest Precipitation" line adds an arrival estimate (e.g. "≈ 4.2 hrs out at current storm speed") when the rain is real but beyond the dial\'s window.'],},
  {ver:'v5.43',date:'2026-07-07',items:['🧩 <b>One shared radar brain</b> — the color-decoding tables and distance math that turn radar pixels into storm intensity used to exist in several separate copies (in the app and in the background alert scanner), which could drift apart over time and cause subtle disagreements. They now live in one shared module used everywhere, so the app and your background push alerts are guaranteed to read the radar identically. No visible changes — this is groundwork for smarter storm tracking coming next.'],},
  {ver:'v5.42',date:'2026-07-07',items:['⏳ <b>3D tab spinner now appears instantly</b> — on first open, the 3D tab could show a blank screen for a few seconds while the 3D engine downloaded, because the page hadn\'t been given its height yet and the loading spinner was hiding in a zero-height area. The tab is now sized immediately on tap, so the spinner shows the moment you switch to it.'],},
  {ver:'v5.41',date:'2026-07-07',items:['🛡️ <b>3D tab error handling hardened</b> — if the 3D engine ever fails to start (for example on devices without 3D graphics support or with strict privacy modes), the tab now shows the clear "3D engine unavailable" message with a Reload &amp; Retry button instead of a spinner that never goes away.'],},
  {ver:'v5.40',date:'2026-07-07',items:['🗻 <b>3D tab fixed</b> — v5.39\'s on-demand 3D loading accidentally erased the 3D page\'s built-in overlays (the loading spinner, the "no location" message, and all on-screen controls) while showing its "Loading 3D…" text, which left the tab stuck. The loader now uses the page\'s own loading spinner and never touches the 3D view\'s controls, so the tab loads normally again — and if the 3D library ever fails to download, you\'ll see a clear error with a Reload &amp; Retry button instead of an endless spinner.'],},
  {ver:'v5.39',date:'2026-07-07',items:['⚡ <b>Faster startup — scripts load in parallel, 3D and Turf deferred</b> — previously all 15 app scripts loaded one-at-a-time before the page could paint, Three.js (the 3D library) downloaded on every launch even if you never opened the 3D tab, and the Turf geometry library loaded at startup when it isn\'t needed at boot. Now: app scripts download in parallel (browsers can fetch all 15 at once), Three.js and the 3D code only download the first time you tap the 3D tab (with a brief "Loading 3D…" while they arrive), and Turf is gone from startup entirely. Every tab behaves identically — this is purely about <em>when</em> things load, not what they do.'],},
  {ver:'v5.38',date:'2026-07-07',items:['🔍 <b>Globe icons sharp again on the Weather hero</b> — v5.37\'s icon-pack slimming sized the media for small icons but missed that the Weather tab\'s big hero showcase draws them at up to 300&nbsp;px. All 88 Globe/Animated-Globe files have been rebuilt from the originals at hero-appropriate resolution (768&nbsp;px stills, 640&nbsp;px square-cropped videos), so the hero icon is crisp on retina phones again — while the packs stay ~80% smaller than the pre-v5.37 originals.'],},
  {ver:'v5.37',date:'2026-07-07',items:['🚀 <b>Globe icon packs load ~95% faster</b> — the 🌐 Globe and 🌐 Animated Globe icon packs were shipping full HD media (up to 9&nbsp;MB per animated icon!) for icons drawn at thumbnail size. All 88 files have been re-encoded to icon-appropriate resolution: the animated pack shrank from 120&nbsp;MB to 3.3&nbsp;MB and the still pack from 45&nbsp;MB to 4.5&nbsp;MB. They look identical on screen but now load quickly and use far less mobile data.'],},
  {ver:'v5.36',date:'2026-07-04',items:['🧠 <b>Choose your AI: OpenAI or Claude</b> — the AI Weather Assistant now lets you pick between <b>OpenAI (GPT)</b> and <b>Claude (Anthropic)</b> right in Settings. Add your own key for whichever you prefer, choose a model, and it powers everything AI: the in-app chat, the AI Briefings, and the optional AI-written background push alerts. OpenAI stays the default, so nothing changes unless you switch. Your key is stored only on your device (and, for background alerts, encrypted on the alert server) and is billed to your own account.'],},
  ...
  {ver:'v5.27',date:'2026-06-19',items:['🔑 <b>AI-written alerts now use your own OpenAI key</b> — when you turn on <b>Settings → Background alerts → AI-written alerts</b>, there’s now a box to paste <b>your own</b> OpenAI key, a <b>Test key</b> button to confirm it works, and a simple step-by-step <b>“How to get a key”</b> guide. Your AI-written notifications are created with your key (billed to your own OpenAI account) and stored securely — never shared with anyone. This keeps AI alerts personal to you. Still completely optional and off by default.'],},
...
function setConeMinDbz(val){
  ...
  toast('🎯 Min storm intensity set to '+v+' dBZ');
}
